...
![Figure 1 — PAN vs 1-layer transformer on modular addition mod 113 (seed=42). PAN uses 172× fewer parameters and matches peak val_acc within 0.14 pp, at the cost of 5.5× more training steps and ~3.3× wall-clock (the phase modulo-add is unoptimized on MPS).](drafts/figs_v5/fig_s31_param_efficiency.png)
...
![Figure 2 — seed=42 ablations. Zeroing phase mixing, randomizing encoder frequencies, or zeroing reference phases each collapses val_acc from 1.00 to < 0.04.](drafts/figs_v5/fig_s32_ablations.png)
...
![Figure 3 — seed=42 metric trajectories on log-step axis. decoder_fourier_peak_mean peaks at 0.87 at step 2,500 — well before grok at step 16,500 — and decays to 0.41 by step 99,500. gate_linear_acc continues to climb after val_acc saturates.](drafts/figs_v5/fig_s32_decoder_fourier_dynamics.png)
...
(min 0.78, max 1.00). **7 of 14 grokked seeds achieve compliance 1.00**
...
![Figure 4 — Slot census across the 14 grokked seeds at P=113, K=9, Fourier init. Panel (a): clock_compliance per seed, mean 0.94, with 7/14 seeds at perfect compliance = 1.00. Panel (b): distribution of active Fourier frequencies per seed (mean 3.79).](drafts/figs_v5/fig_s33_slot_census.png)
...
![Figure 6 — Cross-prime generalization at K=10. Panel (a): per-prime grok rate for each encoder initialization. Panel (b): median grok step over grokked seeds. Random init reaches 24/24 across the eight primes; Fourier init leaves 4 failures at P=89, P=113, P=127.](drafts/figs_v5/fig_s35_cross_prime.png)
...
![Figure 8 — 16-bit SFP phase quantization at inference preserves val_acc to zero measurable loss on the two fully grokked seeds and 0.3pp on the undertrained seed=456.](drafts/figs_v5/fig_s37_sfp16_quant.png)
...
![Figure 5 — K-sweep reliability at P=113, combining both encoder initializations. Error bars are Wilson 95% CIs; shaded bands mark the three regimes. Random init dominates the transition band (K=5–10); both inits converge on the plateau (K≥11).](drafts/figs_v5/fig_s34_k_sweep_reliability.png)
